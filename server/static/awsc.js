!function(t, a) {
  ...
  function E(t, a, r) {
      ...
      if (0 !== s.depends.length)
          for (var T = 0; T < s.depends.length; T++) {
              var y = s.depends[T];
              r && (delete r.sync,
              delete r.timeout,
              delete r.cdn),
  ...
  function P(t, a, r) {
      var s = M(t);
      return s ? (s.state === p || s.state === b ? a && a(s.state, s.exportObj) : s.state === h ? a && s.callbacks.push(a) : void 0,
      void 0) : (E(t, a, r),
      void 0)
  }
  function O(t, a, r) {
      ...
      try {
          ...
          for (var w = void 0, A = 0; A < v.length; A++) {
              var k = v[A];
              k.module === c && k.name === a && (s = k.options && k.options.throwExceptionInCallback,
              w = k,
              clearTimeout(w.timeoutTimer),
              delete w.timeoutTimer,
              w.exportObj = r,
              k.state === h || k.state === b ? (w.state = p,
              x(w, s)) : void 0)
          }
          w || (w = {},
          w.module = c,
